...
def load_planner_metas_advanced(file, data_ref):
    try:
        ...
        # Mapeamento conforme solicitado: Linha no Excel - 1 para índice Pandas
        ...
        if target_col is not None:
            for maq, row_idx in maq_lines.items():
                # Planejado do dia
                plan_dia[maq] = pd.to_numeric(df_raw.iloc[row_idx, target_col], errors='coerce') or 0
                # Planejado acumulado mês
                soma_acum = 0
                for i, d in enumerate(row_dates):
                    if isinstance(d, (datetime, pd.Timestamp)) and d.month == data_ref.month and d.year == data_ref.year and d.date() <= data_ref:
                        soma_acum += pd.to_numeric(df_raw.iloc[row_idx, i], errors='coerce') or 0
                plan_mes_acum[maq] = soma_acum
        
        # Meta Geral Linha 125
        row_meta_125 = df_raw.iloc[124, :].tolist()
        m_geral, m_acum_hoje = 0, 0
        for i, d in enumerate(row_dates):
            if isinstance(d, (datetime, pd.Timestamp)):
                val = pd.to_numeric(row_meta_125[i], errors='coerce') or 0
                m_geral += val
                if d.date() <= data_ref: m_acum_hoje += val
                
        return plan_dia, plan_mes_acum, m_geral, m_acum_hoje
    except: return {}, {}, 0, 0
...
if uploaded_file:
    ...
    if menu == "📋 REPORTE DIÁRIO":
        ...
        plan_dia, plan_mes_acum, m_geral, m_hoje = load_planner_metas_advanced(up_datas, data_ref_reporte) if up_datas else ({}, {}, 0, 0)
        ...
        st.markdown(f"""
            ...
            <div class="metric-container">
                <div class="metric-card"><div class="metric-title">Meta Acumulada (Até {data_ref_reporte.strftime('%d/%m')})</div><div class="metric-value" style="color:#3b82f6">{m_hoje:,.0f}</div></div>
                <div class="metric-card"><div class="metric-title">Meta Geral do Mês (Datas)</div><div class="metric-value" style="color:#10b981">{m_geral:,.0f}</div></div>
            </div>
        """, unsafe_allow_html=True)
        ...
        with col_t1:
            st.markdown("**Último Dia Selecionado**")
            ...
            for m in sorted(df_order['Máquina'].unique(), key=int):
                ...
                res_gap_dia.append({'Máquina': m, 'Realizado': r, 'Planejado': p, 'Gap/Ganho': r-p})
        ...
        with col_t2:
            ...
            for m in sorted(df_order['Máquina'].unique(), key=int):
                ...
                res_gap_mes.append({'Máquina': m, 'Realizado Mês': r, 'Planejado Mês': p, 'Gap/Ganho Mês': r-p})
    ...
    # =========================================================
    # ABA: TOP 10 PARADAS (FUNDO BRANCO)
    # =========================================================
    ...
    # =========================================================
    # ABA: ANÁLISE SEMANAL (FUNDO BRANCO)
    # =========================================================
        ...
        df_b_all = df_order[(df_order['Data'].dt.date >= per[0]) & (df_order['Data'].dt.date <= per[1]) & (df_order['Turno'].isin(turn_sel))]
        ...
        st.markdown(f"""<div style="text-align:center; border:2px solid #10b981; padding:10px; border-radius:10px;"><h2 style="color:#0f172a; margin:0;">RELATÓRIO SEMANAL - MÁQUINA {maq_sel}</h2><p style="color:#64748b;">{per[0].strftime('%d/%m')} a {per[1].strftime('%d/%m/%Y')}</p></div>""", unsafe_allow_html=True)
        ...
        with col_r:
            rk = df_b_all.groupby('Máquina').agg({'Run Time':'sum','Horário Padrão':'sum'}).reset_index(); rk['Mov %'] = (rk['Run Time']/rk['Horário Padrão'].replace(0,1)*100).round(1); rk = rk.sort_values('Mov %', ascending=False).reset_index(drop=True); rk.index += 1
            ...
            msg, cor = ("🏆 Liderança semanal!", "#dcfce7") if pos <= 2 else ("🚀 Foco para subir!", "#fee2e2")
